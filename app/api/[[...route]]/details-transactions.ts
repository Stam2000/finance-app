...
const app = new Hono()
    .get("/",zValidator("query",z.object({
        ...
        .where(and(eq(detailsTransactions.transactionId,transactions.id),
...
.get("/:id",
    ...
    async(c)=>{
        ...
        .where(
            and(
                inArray(detailsTransactions.id,values.ids),
            )
        ).returning({
            id:detailsTransactions.id,
        });
    ...
    zValidator("json",
       z.array( insertdetailsTransactionsSchema.omit({
            id:true
        }))
    ),
    async(c)=>{
        ...
        const data = await db.insert(detailsTransactions)
    ...
    zValidator("json",
        insertdetailsTransactionsSchema.omit({
            ...
            const transactionToUpdate = db.$with("deetailsTransactions_to_update")
                .as(
                    db.select({id:detailsTransactions.id})
                    .from(detailsTransactions)
            ...
            const [data] = await db.with(transactionToUpdate).
                ...
                .set(values)
                .where(
                    inArray(detailsTransactions.id,sql`(select id from ${transactionToUpdate})`)
                ).returning()
...
delete("/:id",
    ...
    async(c)=>{
        ...
        const transactionToDelete = db.$with("deetailsTransactions_to_update")
        .as(
            db.select({id:detailsTransactions.id})
            .from(detailsTransactions)
            .innerJoin(transactions,eq(transactions.id,detailsTransactions.transactionId))
        ...
        const[data] = await db.with(transactionToDelete)
            ...
            .where(
                inArray(
                    detailsTransactions.id,sql`(select id from ${transactionToDelete})`
                )
            ).returning({
                id:detailsTransactions.id
            })
