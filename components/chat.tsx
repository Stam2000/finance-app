...
export const Chat = () =>{
    ...
    const  isSubmitDisabled  = formData.question.trim()==="" && formData.Files.length === 0;
   
    return(
        <div className={`fixed  z-50 md:bottom-2 top-2 left-2 md:left-auto bottom-2 border-black right-0 md:top-2 overflow-hidden bg-slate-100
                    ...
                    <div className=" px-3 py-2 flex  items-start gap-2 justify-between absolute top-0 left-0 right-0 ">
                        <div className="text-xl items-center justify-between text-slate-900 flex gap-2 h-full">
                            ...
                            <button onClick={newChatCreate} className="flex shadow-md hover:bg-slate-900 hover:text-white border-[1px] border-slate-300 bg-slate-300 cursor-pointer items-center gap-2 text-xs text-slate-900 py-2 px-4 rounded-sm" >
                                New Chat
                            </button>
