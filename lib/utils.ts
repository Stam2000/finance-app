...
export const sendAiMessage = async ({threadId,personaId,setThreadId,updateLastMessage,updateMessage,formData,message,setFormData,setIsLoading,personaInfo}:{
...
export async function uploadEntitiesAndReplaceReferences(Bigdata:z.infer<typeof formSchema>|string,personaId:string) {
...
const uploadAccountPromises = data.accounts.map((account) =>limit( async () => {
  ...
  if (accountNameToIdMap.has(account.name)) {
    ...
    const res =  await client.api.accounts.$post({ json: account },{headers: {
      'X-Persona-ID': personaId,  
  }});
...
const uploadCategoryPromises = data.categories.map((category) =>limit( async()=> {
...
const uploadProjectPromises = data.projects.map((project) =>limit(async()=> {
...
// Await all uploads
await Promise.all([...uploadAccountPromises, ...uploadCategoryPromises, ...uploadProjectPromises]);
...
export class MarkdownFormatter {
  ...
  private static formatCodeBlocks(text: string): string {
    ...
    text = text.replace(/```(\w+)?\n([\s\S]+?)```/g, 
      (_, lang, code) => `<pre><code class="language-${lang || 'plain'}">${this.escapeHtml(code.trim())}</code></pre>`
    );
